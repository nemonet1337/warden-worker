...
#   D1_DATABASE_ID        (optional)  production D1 database id; only required when wrangler.toml
#                                     still contains the ${D1_DATABASE_ID} placeholder
...
#   R2_NAME               (optional)  R2 bucket name; enables the ATTACHMENTS_BUCKET binding when missing
#
set -euo pipefail
...
if grep -q '\${D1_DATABASE_ID}' wrangler.toml; then
  ...
  echo "wrangler.toml already has a concrete D1 database_id; skipping substitution"
fi

if [ -n "${R2_NAME:-}" ]; then
  if grep -q '^binding = "ATTACHMENTS_BUCKET"' wrangler.toml; then
    echo "R2 binding already present in wrangler.toml; skipping"
  else
